...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("col-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="col-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="col-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="col-pet">' +
    ...
    settingRow("NAP", petToggle("col-pet-nap")) +
    settingRow("FLEE", petToggle("col-pet-flee")) +
    settingRow("READ", petToggle("col-pet-read")) +
    settingRow("TRICKS", petToggle("col-pet-tricks")) +
    settingRow("SPEECH", petToggle("col-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "col-pet": "float", "col-pet-nap": "on", "col-pet-flee": "on",
      "col-pet-read": "on", "col-pet-tricks": "on", "col-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "col-theme") cur = TW.prefs.get("col-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("col-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("col-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("col-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "col-theme") {
      if (val === "system") { TW.prefs.remove("col-theme"); }
      else { TW.prefs.set("col-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "col-accent") {
      if (val === "0") { TW.prefs.remove("col-accent"); }
      else { TW.prefs.set("col-accent", val); }
